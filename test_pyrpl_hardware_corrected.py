#!/usr/bin/env python3
"""
Corrected PyRPL Hardware Test with Proper Parameter Structures

This script tests PyRPL plugins with the correct parameter paths for each plugin.
"""
...
def test_pyrpl_plugins():
    """Test PyRPL plugins with correct parameter structures."""
    ...
    try:
        ...
        # Configure for real hardware (connection_settings)
    ...
    try:
        ...
        # Configure for real hardware (connection)
        voltage_plugin.settings.child('connection', 'redpitaya_host').setValue('rp-f08d6c.local')
        voltage_plugin.settings.child('connection', 'mock_mode').setValue(False)
    ...
    try:
        ...
        # Configure for real hardware (connection_settings + dev_settings)
        asg_plugin.settings.child('connection_settings', 'redpitaya_host').setValue('rp-f08d6c.local')
        asg_plugin.settings.child('dev_settings', 'mock_mode').setValue(False)
    ...
    try:
        ...
        # Configure for real hardware (connection)
        scope_plugin.settings.child('connection', 'redpitaya_host').setValue('rp-f08d6c.local')
        scope_plugin.settings.child('connection', 'mock_mode').setValue(False)
    ...
    try:
        ...
        # Configure for real hardware (connection)
        iq_plugin.settings.child('connection', 'redpitaya_host').setValue('rp-f08d6c.local')
        iq_plugin.settings.child('connection', 'mock_mode').setValue(False)
...
def main():
    """Main test function."""
    logger.info("🔬 PyMoDAQ PyRPL Plugin Hardware Validation (Corrected)")
